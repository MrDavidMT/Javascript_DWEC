...
class Instructor extends Lambdasian{
    ...
    puntuar(student){
        if(Math.round(Math.random())==0){
            student.grado+=Math.round(Math.random()*(100-student.grado));
        }else{
            student.grado-=Math.round(Math.random()*student.grado);
        }
...
class ProjectManager extends Instructor{
    ...
    standUp(){return `${this.name} announces to {channel}, @channel standy times!`}
    debusCode(student,subject){return `student: ${student.name} and subject: ${subject}`}
    puntuar(student){super.puntuar(student);}
}
